...
def create_data_part(opt):
    train_data = CPCDataset(opt.path_to_train_csv,opt.path_to_imgs)
    test_data = CPCDataset(opt.path_to_test_csv,opt.path_to_imgs)
    val_data = CPCDataset(opt.path_to_val_csv,opt.path_to_imgs)
...
def validate(opt,model, loader, criterion, writer=None, global_step=None, name=None):
    ...
    true_score_lable = np.where(true_score <= 2.00, 0, 1)
    pred_score = np.array(pred_score)
    pred_score_lable = np.where(pred_score <= 2.00, 0, 1)
